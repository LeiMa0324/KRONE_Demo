  ...
  useEffect(() => {
    ...
    const node = svg.append("g")
      ...
      .on("click", function (event: MouseEvent, d: HierarchyNode<TreeNode>) {
        ...
        if (!collapsible || !d.data.indexPath) return;
        setLocalTree(prev => {
          if (!prev) return null;
    ...
    // Highlight matched node and relatives
    if (matchedNodeId) {
      const matched = root.descendants().find(
        d => d.data.name === matchedNodeId || (d.data.event_id && d.data.event_id === matchedNodeId)
      );
      if (matched) highlightRelated(svg, matched);
    }
  }, [
    localTree,
    matchedNodeId,
